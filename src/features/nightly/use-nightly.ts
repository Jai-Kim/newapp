...
/** What tonight's screen shows when the spend guard (issue #6) says not yet. */
  ...
  return React.useCallback(
    async (lesson: string | undefined, situation: string | undefined) => {
      ...
      try {
        ...
        // A quota block is not a failure to explain away — it is the spend
        // guard working as designed, so it gets its own warm, bilingual
        // notice rather than the generic red error banner.
        if (e instanceof GenerationQuotaError) {
          setQuotaNotice({ code: e.code, messageEn: e.messageEn, messageKo: e.messageKo });
        }
        else {
